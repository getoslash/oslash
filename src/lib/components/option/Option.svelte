<script lang="ts">
  ...
  $: resetSearchQuery(savedShortcuts);

  /**
   * listener function when shortcuts stored in local storage changes
   * @returns {void} void
   */
  async function localStorageChangeListener(
    changes: {
      [key: string]: chrome.storage.StorageChange;
    },
    area: chrome.storage.AreaName
  ) {
    if (area === "local" && changes[SHORTCUTS_STORAGE_KEY].newValue) {
      loadSavedShortcuts();
    }
  }

  /**
   * loads the latest saved shortcuts
   * @returns {void} void
   */
  ...
  function exportShortcutsToCSV() {
    ...
    const shortcutData = savedShortcuts.map((shortcut) => {
      return {
                      ...
                      <td class="p-2.5 text-sm text-gray-500">
                        ...
                        >
                          ...
                          <svg
                            ...
                            ><path
                              d="M2.5 11.64v2.027a.33.33 0 0 0 .333.333H4.86a.313.313 0 0 0 .233-.1l7.28-7.273-2.5-2.5L2.6 11.4a.327.327 0 0 0-.1.24Zm11.807-6.947c.26-.26.26-.68 0-.94l-1.56-1.56a.664.664 0 0 0-.94 0l-1.22 1.22 2.5 2.5 1.22-1.22Z"
                            /></svg
                          >
                        </button>
                      </td>
